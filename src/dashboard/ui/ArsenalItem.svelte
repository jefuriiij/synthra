...
>
  <div class="flex items-center gap-2">
    <span class="min-w-0 flex-1 truncate font-mono text-[12px] text-foreground">{item.name}</span>
    <span
      class="shrink-0 rounded border px-1.5 py-0.5 font-mono text-[8px] uppercase tracking-wide"
  ...
  {#if item.description}
    <p class={"text-[11.5px] leading-snug text-muted-foreground " + (open ? "" : "line-clamp-2")}>
      {item.description}
    </p>
  {/if}
  {#if open && meta.length}
    <div class="mt-1 flex flex-col gap-0.5 font-mono text-[10px] text-muted-foreground/80">
      {#each meta as [k, v] (k)}
