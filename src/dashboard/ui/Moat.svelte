...
<Card title="The Moat" meta="PreToolUse">
  ...
  <div class="flex max-h-[320px] flex-col gap-1 overflow-y-auto">
    {#each gates as g (g.ts + g.query)}
      <div class="flex items-baseline gap-2 font-mono text-[10px]">
        <span class="shrink-0 text-muted-foreground">{fmtTs(g.ts)}</span>
        <span
          class={"shrink-0 rounded px-1 text-[9px] uppercase " +
    ...
    {:else}
      <div class="text-[11px] text-muted-foreground">No gate decisions yet.</div>
    {/each}
  </div>
</Card>
